...
window.sendMessage = async function() {
    ...
    const assistantMsg = addChatMessage('assistant', '', true);
    ...
    try {
        ...
        assistantMsg.classList.remove('temporary');
        assistantMsg.querySelector('.message-time').textContent = new Date().toLocaleTimeString();
        updateChatStats();

    } catch (error) {
        assistantMsg.classList.remove('temporary');
        textEl.textContent = '抱歉，' + error.message;
    }
};
